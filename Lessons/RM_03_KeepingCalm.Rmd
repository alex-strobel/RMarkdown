...
apa_print.corr.test <- function(df,method="pearson",classify="gignac") {
  ...
  for (i in 1:nrow(ci)) {
    if (classify == "gignac") {
      if (ci$r[i] < .2) {es="small"} else if (ci$r[i] >= .2 & ci$r[i] <= .3) {es="medium"} else {es="large"}
    } else if (classify == "cohen") {
      if (ci$r[i] < .3) {es="small"} else if (ci$r[i] >= .5 & ci$r[i] <= .5) {es="medium"} else {es="large"}
    }
    r=sub("[0]+", "", format(round(ci$r[i],2),nsmall=2))
    # format p-value
    if (ci$p[i] > .001) {p = paste("_p_ = ",sub("[0]+", "", format(round(ci$p[i],3),nsmall=3)),sep="")} else {p = "_p_ < .001"}
    # format ci
...
vio.plot <- function(df,ylim=NULL) {
  if (is.null(ylim)) {ylim=c(min(df)-1,max(df)+1)}
...
distances=mahalanobis(df,center=colMeans(df),cov=covMcd(df)$cov) # squared Mahalanobis distances (using a robust estimate of covariance from the robustbase package)
